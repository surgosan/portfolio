...
module.exports = {
    async inputData (req, res) {
        try {
            ...
            const createdData = await log.bulkCreate(inputData, { updateOnDuplicate: ['datetime'] });
            res.send(`${createdData.length} logs submitted. ${inputData - createdData} duplicates avoided.`);
        } catch (error) {
